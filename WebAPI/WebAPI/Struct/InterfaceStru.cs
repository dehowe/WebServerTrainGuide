namespace WebAPI.Struct
{
    ...
    {
        public long ScheduleID { get; set; }                    // 时刻表编号
    ...
    //接口7.2.2 请求结构体定义
    public struct Request722
    {
        public string ShiftCode { get; set; }                    // 班次号
        public DateTime StartTime { get; set; }                    // 班次计划开始时间
        public string StartingPlatformCode { get; set; }                    // 始发站
        public string TerminalPlatformCode { get; set; }                    // 终点站
        public int Direction { get; set; }                    // 运行方向	Int	1：上行（逆时针）；2：下行（顺时针）
        public int OperatingScale { get; set; }                    // 运行标尺
        public int StopScale { get; set; }                    // 停站标尺
    }

    //接口7.2.2 回复结构体定义
    public struct Response722
    {
    ...
    //接口7.2.3 回复结构体定义
    public struct Response723
    {
        public string ScheduleID { get; set; }                    // 时刻表编号
        public List<Shift722> ShiftList { get; set; }                // 全天班次数据集合
    }

    //接口7.2.4 请求结构体定义
    public struct Request724
    {
        public List<Shift724> ShiftList { get; set; }               // 需调整班次及其后续班次数据集合
    }

    //接口7.2.4 返回结构体定义
    public struct Response724
    {
        public string ScheduleID { get; set; }                    // 时刻表编号
        public List<Shift722> ShiftList { get; set; }                // 全天班次数据集合
    }
    ...
    {
        public DateTime RequestTime { get; set; }                      // 请求时间
        public List<CarStatus> CarStatusList { get; set; }           // 车辆相关状态集合
        public List<StationStatus> StationStatusList { get; set; }   // 站台占用状态集合
        //public List<SectionStatus> SectionStatusList { get; set; }   // 区段占用状态集合
    }
    ...
    //接口7.2.6 请求结构体定义
    public struct Request726
    {
        public DateTime RequestTime { get; set; }                      // 请求时间

        public List<CarStatus726> CarStatusList { get; set; }           // 车辆相关状态集合
    }

    //接口7.2.6 回复结构体定义
    public class Response726
    {
        public List<CarGuideData726> CarGuideDataList = new List<CarGuideData726>(); // 车辆指导数据集合
    }
    ...
    public struct ShiftDetail                       // 班次明细列表集合结构体
    {
        ...
        public DateTime InTime { get; set; }                        // 计划进站时间
        public DateTime OutTime { get; set; }                       // 计划出站时间
    }

    //接口7.2.4 内部嵌套结构体定义
    public struct Shift724                             // 全天班次数据集合结构体
    {
        public string ShiftCode { get; set; }                     // 班次号
        public int Direction { get; set; }                        // 运行方向	Int	1：上行（逆时针）；2：下行（顺时针）
        public DateTime StartTime { get; set; }                     // 班次计划开始时间
    ...
    public class CarStatus                         // 车辆相关状态集合结构体
    {
        public string CarCode { get; set; }                      // 车辆编号
        public string ShiftCode { get; set; }                    // 班次号
        public int ShiftType { get; set; }                    // 班次类型	String	1：常规运营班次；2：救援班次；3：VIP接待班次；4：调车班次
        public int Direction { get; set; }                       // 运行方向	Int	1：上行（逆时针）；2：下行（顺时针）
    ...
    public struct CarStatus726                         // 车辆相关状态集合结构体
    {
        public string CarCode { get; set; }                      // 车辆编号
        public string ShiftCode { get; set; }                    // 班次号
        public int ShiftType { get; set; }                    // 班次类型	String	1：常规运营班次；2：救援班次；3：VIP接待班次；4：调车班次
    ...
    public struct CarGuideData726                      // 车辆指导数据集合结构体
    {
        public string CarCode { get; set; }                      // 车辆编号
        public int AccessState { get; set; }                     // 准入状态	Int	1：允许通行；2：禁止通行
        public string AccessFailureResult { get; set; }          // 准入失败原因
    }
